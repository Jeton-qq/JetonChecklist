...
local function BottomFor(obj)
    ...
    local y = tonumber(obj._JetonBaseY) or 0
    local h = 0
...
local function IsMainFrameOnListTab()
    local main = _G and _G["JetonChecklistFrame"]
    local selectedTab = main and tonumber(main._JetonSelectedTab)
...
local function SetRightRowPair(i, rowLabel, rowValue, iconFileID, currencyID, tooltipText, amountTooltipText)
    ...
    if row.frame then
        row.frame._JetonTooltipText       = tooltipText or nil
        row.frame._JetonAmountTooltipText = amountTooltipText or nil
    end
    if row.icon then
        if showRow and iconFileID and iconFileID ~= 0 then
            if row.icon._tex then row.icon._tex:SetTexture(iconFileID) end
            row.icon._JetonIconCurrencyID = currencyID or nil
            SetShownIfChanged(row.icon, true)
        else
            row.icon._JetonIconCurrencyID = nil
...
local function ResizeTrackingPanelToContent(addon)
    ...
    if trackingFrame._JetonLeftCol  and trackingFrame._JetonLeftCol.SetHeight  then trackingFrame._JetonLeftCol:SetHeight(max(1, targetH - 40))  end
    if trackingFrame._JetonRightCol and trackingFrame._JetonRightCol.SetHeight then trackingFrame._JetonRightCol:SetHeight(max(1, targetH - 40)) end
...
local function EnsureTrackingPanelCreatedIfNeeded(wantPanel)
    ...
    local main = _G["JetonChecklistFrame"]
    if main then
...
function Addon:CreateTrackingPanel(parentFrame)
    ...
    trackingFrame._JetonLeftTitle = title

    local padL, padR = 10, 10
    local colGap = 12
    local innerW = (UI.frameW - (UI.sectionInsetX * 2) - padL - padR)
    local colW   = math.floor((innerW - colGap) / 2)
    trackingFrame._JetonPadL    = padL
    trackingFrame._JetonPadR    = padR
    trackingFrame._JetonColGap  = colGap
    trackingFrame._JetonColW    = colW
    ...
    trackingFrame._JetonLeftCol = leftCol
    ...
    trackingFrame._JetonRightCol = rightCol
    ...
    trackingFrame._JetonLeftBox  = leftBox
    trackingFrame._JetonRightBox = rightBox
    ...
    trackingFrame._JetonRightTitle = rightTitle
    ...
    trackingFrame._JetonGvGrids = gvGrids
    ...
    local function ReflowGVGrid(targetH)
        ...
        for bi = 1, 3 do
            ...
            local function setHL(t, y)
                ...
                t:SetWidth(gridW); t._JetonBaseY = y
            end
    ...
    local function MakeLinePair(parent, y, template)
        ...
        row:SetHeight(16); row._JetonBaseY = y
        ...
        icon:SetScript("OnEnter", function(self)
            if self._JetonIconCurrencyID then
                GameTooltip:SetOwner(self, "ANCHOR_LEFT")
                GameTooltip:SetCurrencyByID(self._JetonIconCurrencyID)
                GameTooltip:Show()
            end
        end)
        ...
        valueHit:SetScript("OnEnter", function(self)
            local tip = row._JetonAmountTooltipText
            if tip and tip ~= "" then
                GameTooltip:SetOwner(self, "ANCHOR_TOP")
        ...
        row:SetScript("OnEnter", function(self)
            local tip = self._JetonTooltipText
            if tip and tip ~= "" then
                GameTooltip:SetOwner(self, "ANCHOR_TOP")
                GameTooltip:SetText(tip, 1, 1, 1, 1, true)
                GameTooltip:Show()
...
function Addon:ApplyTrackingPanelOptions()
    ...
    self:ConfigureTrackingEvents(_G["JetonChecklistFrame"], showGreatVault, showCurrency)

    local leftCol    = trackingFrame._JetonLeftCol
    local rightCol   = trackingFrame._JetonRightCol
    local leftTitle  = trackingFrame._JetonLeftTitle
    local rightTitle = trackingFrame._JetonRightTitle
    local padL   = tonumber(trackingFrame._JetonPadL)   or 10
    local padR2  = tonumber(trackingFrame._JetonPadR)   or 10
    local colGap = tonumber(trackingFrame._JetonColGap) or 12
    ...
    SetShownIfChanged(trackingFrame._JetonLeftBox,  showBothBoxes)
    SetShownIfChanged(trackingFrame._JetonRightBox, showBothBoxes)
    ...
    if showGreatVault and showCurrency then
        trackingFrame._JetonShowBoth = true
        ...
        trackingFrame._JetonShowBoth = false
        if showGreatVault then
...
function Addon:ResizeTrackingCols()
    ...
    local padL    = tonumber(tf._JetonPadL)   or 10
    local padR    = tonumber(tf._JetonPadR)   or 10
    local colGap  = tonumber(tf._JetonColGap) or 12
    local leftCol  = tf._JetonLeftCol
    local rightCol = tf._JetonRightCol
    ...
    local leftTitle  = tf._JetonLeftTitle
    local rightTitle = tf._JetonRightTitle
