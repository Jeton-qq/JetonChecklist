...
local function BuildIlvlRefWindow()
    ...
    -- Starting points and rank offsets are defined in JetonChecklist_Constants.lua
    ...
    local win = Addon:NewThemedFrame("JetonIlvlRefFrame", UIParent)
    ...
    tinsert(UISpecialFrames, "JetonIlvlRefFrame")
...
function Addon:RebuildIlvlRefWindow()
    ...
    if _G["JetonIlvlRefFrame"] then
        _G["JetonIlvlRefFrame"] = nil
    end
    if wasShown then
        self._ilvlRefWindow = BuildIlvlRefWindow()
        self._ilvlRefWindow:Show()
