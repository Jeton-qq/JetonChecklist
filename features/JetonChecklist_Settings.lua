...
-- Jeton_COPY_LINK popup can reliably display it (self.data can be nil
...
StaticPopupDialogs["Jeton_LOCALE_RELOAD"] = StaticPopupDialogs["Jeton_LOCALE_RELOAD"] or {
...
StaticPopupDialogs["Jeton_COPY_LINK"] = {
            ...
            eb:SetScript("OnKeyDown", function(_, key)
                if key == "C" and IsControlKeyDown() then
                    C_Timer.After(0.05, function()
                        StaticPopup_Hide("Jeton_COPY_LINK")
                    end)
                end
            end)
        end)
...
function Addon.OpenSupportLink(url)
    ...
    if C_Browser and C_Browser.OpenLink then
        ...
        StaticPopup_Show("Jeton_COPY_LINK", nil, nil, url)
    end
end
...
local function BuildPanel()
    ...
    panelFrame.name = L.DISPLAY_NAME or "Jeton's Weekly Checklist"
    ...
    titleFS:SetText(L.DISPLAY_NAME or "Jeton's Weekly Checklist")
    curY = curY - 32
    ...
    local function GetOrBuildLangPopup()
        ...
        for idx, opt in ipairs(LOCALE_OPTIONS) do
            ...
            row:SetScript("OnClick", function()
                ...
                if Addon._styleActionButton then Addon._styleActionButton(langDropBtn) end
                StaticPopup_Show("Jeton_LOCALE_RELOAD")
            end)
        end
        return langPopup
    end
    ...
    langDropBtn:SetScript("OnClick", function()
        ...
        if p._JetonClosedAt and (GetTime() - p._JetonClosedAt) < 0.20 then p._JetonClosedAt = nil; return end
    ...
    -- back to the Jeton_COPY_LINK clipboard popup.
