...
do
    ...
    function Addon:InitConstants(addonNameInput)
        ...
        if names.accountDbName == nil then names.accountDbName = "JetonChecklistDB" end
        ...
        -- Tracking IDs are sourced from `JetonChecklist_Constants.lua`.
        -- This keeps one obvious edit spot for currency/achievement/quest IDs.

        -- Optional user overrides (IDs, tracking settings, etc.)
        -- Loaded from `JetonChecklist_Constants.lua` via _G["<addonName>_CONSTANTS"].
...
local LOCALIZATION_ADDON_NAME = "JetonChecklist_Localization"
Addon.LOCALIZATION_COMPANION_HINT_TEXT = Addon.LOCALIZATION_COMPANION_HINT_TEXT
    or "Tip: For non-English translations, install the optional addon 'JetonChecklist: Localization'."
...
local function SetupMinimapIcon()
    ...
    local dataObject = LDB:NewDataObject(addonName, {
        type = "data source",
        text = addonName,
        icon = "Interface\\AddOns\\JetonChecklist\\assets\\icon",
...
function JetonChecklist_CompartmentClick(_, button, down)  -- luacheck: ignore 212
    local now = GetTime()
...
function JetonChecklist_CompartmentOnEnter(_, frame)
...
function JetonChecklist_CompartmentOnLeave()
    GameTooltip:Hide()
end
...
function Addon:EnsureDB()
    ...
    if not cdb._JetonDefaultsApplied then
        ...
        cdb._JetonDefaultsApplied = true
    end
    return cdb
end
...
-- PruneObsoleteSavedState → features/body/JetonChecklist_ListData.lua
...
-- GetListData → features/body/JetonChecklist_ListData.lua
...
function Addon:UpdateLocalizedUI()
    ...
    local changeWeekBtn = frame._JetonChangeWeekBtn
    ...
    local ilvlRefBtn = frame._JetonIlvlRefBtn
    ...
    if trackingFrame then
        if trackingFrame._JetonLeftTitle and trackingFrame._JetonLeftTitle.SetText then
            trackingFrame._JetonLeftTitle:SetText(L.TRACKING_GREAT_VAULT_TITLE or "Great Vault")
        end
        if trackingFrame._JetonRightTitle and trackingFrame._JetonRightTitle.SetText then
            trackingFrame._JetonRightTitle:SetText(L.TRACKING_CURRENCY_TITLE or "Currency")
        end
        ...
        local grids = trackingFrame._JetonGvGrids
...
function Addon:ApplyThemeColors()
    ...
    if tf then
        ...
        if tf._JetonLeftTitle  then tf._JetonLeftTitle:SetTextColor(h.r, h.g, h.b, h.a)  end
        if tf._JetonRightTitle then tf._JetonRightTitle:SetTextColor(h.r, h.g, h.b, h.a) end
        -- Refresh Great Vault row labels (Raid / Dungeons / World) with the current header color.
        local grids = tf._JetonGvGrids
        if type(grids) == "table" then
            for i = 1, 3 do
                local g = grids[i]
    ...
    if _mf and self._styleActionButton then
        if _mf._JetonChangeWeekBtn then self._styleActionButton(_mf._JetonChangeWeekBtn) end
        if _mf._JetonIlvlRefBtn    then self._styleActionButton(_mf._JetonIlvlRefBtn)    end
    end
    ...
    -- Guard with _JetonHeaderPicker to avoid eagerly building the picker frame during color drags.
    local _pickerFrame = _mf and _mf._JetonHeaderPicker
...
-- Footer methods are defined in features/footer/JetonChecklist_Footer.lua.
...
function Addon:UpdateCompletionEasterEgg(db)
    ...
    local pig = frame._JetonPigTexture
...
local function CalcDataSig(data)
    ...
    -- NOTE: This assumes list data doesn't mutate in-place without clearing __JetonSig.
    local cached = rawget(data, "__JetonSig")
...
local GUIDE_LINK = "|cffffd700|HJeton:guide|h[CHECK GUIDE]|h|r"
...
-- to features/header/JetonChecklist_Header.lua.
...
local function RebuildDataIndex(data, sig)
    ...
    if frame and frame._JetonHeaderPicker and Addon._PopulateHeaderPicker then
        Addon._PopulateHeaderPicker()
    end
    return true
end
...
function Addon:CreateFrame()
    ...
    frame = CreateFrame("Frame", "JetonChecklistFrame", UIParent)
    ...
    frame:SetScript("OnHide", function()
        ...
        local picker = frame._JetonHeaderPicker
    ...
    tinsert(UISpecialFrames, "JetonChecklistFrame")
    ...
    -- Defined in features/header/JetonChecklist_Header.lua.
...
-- NOTE: We intentionally register /jtn as a second alias of the *same* command
-- name to avoid collisions with other addons that may use a generic "JTN"
-- SlashCmdList entry.
function Addon:RegisterConsoleCommands()
    ...
    SLASH_JetonCHECKLIST1 = "/Jeton"
    SLASH_JetonCHECKLIST2 = "/jtn"

    local addon = self
    SlashCmdList["JetonCHECKLIST"] = function(input)
...
function Addon:ToggleCommand(input)
    ...
    self:Print(L.SLASH_USAGE_TOGGLE or "Usage: /Jeton or /jtn to toggle the checklist")
end
